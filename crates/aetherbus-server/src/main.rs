...
use aetherbus_audit::{AuditDecision, AuditOutcome, AuditSink, DraftAuditEvent};
use aetherbus_auth::{AuthSnapshot, CapabilityVerifier, RevocationList, VerifiedCapability};
use aetherbus_policy::PolicyEngine;
use aetherbus_protocol::{
...
use aetherbus_storage::{Storage, StorageConfig};
use anyhow::Context;
use clap::Parser;
...
#[derive(Debug, Parser)]
struct Cli {
    #[arg(long, default_value = "configs/aetherbus.example.toml")]
...
fn init_tracing(log_level: &str) -> anyhow::Result<()> {
    let filter = EnvFilter::try_new(log_level)
        .or_else(|_| EnvFilter::try_new(format!("aetherbus_server={log_level}")))
        ...
        TransportKind::Unix => {
            #[cfg(unix)]
            {
                let socket_path = config
                    .socket_path
                    .clone()
                    .unwrap_or_else(|| std::path::PathBuf::from("./tmp/aetherbus.sock"));
...
async fn handle_publish(
    ...
    capability_token: String,
    topic: String,
    classification: Option<aetherbus_protocol::Classification>,
    payload: String,
) -> ControlResponse {
...
mod tests {
    ...
    use aetherbus_policy::{DefaultDecision, PolicyConfig, Rule};
    use aetherbus_protocol::{
    ...
    fn test_root() -> PathBuf {
        std::env::temp_dir().join(format!("aetherbus-server-{}", Uuid::now_v7()))
    }
    ...
    async fn auth_state_and_revocation_admin_commands_work() {
        ...
        match auth_state {
            ControlResponse::AuthState { state } => {
                assert_eq!(state.audience, "aetherbus");
